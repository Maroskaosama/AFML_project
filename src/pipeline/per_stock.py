...
def run_per_stock_pipeline(
    ...
    cusum_start : if given, CUSUM events are only generated from this date
                  onwards.  Features and fracdiff still use the full raw
                  history so the FFD warmup window is served by earlier data.
    ...
    # ── Stage 1: CUSUM filter ──────────────────────────────────────────────
    # Calibrate h on the sub-series starting from cusum_start so event
    # density targets the intended study window, not the full raw history.
    close_for_cusum = (
        close[close.index >= pd.Timestamp(cusum_start)]
        if cusum_start else close
    )

    h = calibrate_cusum_h(close_for_cusum, target_events=target_events)
    cusum_events = cusum_filter(close_for_cusum, h)

    # Safety: widen if < 100 events; narrow if > 800
